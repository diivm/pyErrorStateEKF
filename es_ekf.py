...
var_gnss = 0.1#1#0.5#0.01
var_lidar = 2.5#1#0.25
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    Kal = p_cov_check@h_jac.T@np.linalg.pinv(h_jac@p_cov_check@h_jac.T + sensor_var*np.eye(3))
    ...
    q_check=Quaternion(axis_angle=delta_x[6:9]).quat_mult(Quaternion(*q_check))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k]=p_est[k-1] + delta_t*v_est[k-1] + ((delta_t**2)*0.5)*(Cns@(imu_f.data[k-1]) - g )
    
    v_est[k]=v_est[k-1] + delta_t*(Cns@(imu_f.data[k-1]) - g )

    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1]*delta_t).quat_mult(Quaternion(*q_est[k-1]))
    ...
    # f_jac[3:6,6:9]=-skew_symmetric((Cns@f_last))*delta_t
    f_jac[3:6,6:9] = -skew_symmetric(imu_f.data[k-1])@Cns*delta_t


    a=delta_t**2 * var_imu_f
    b=delta_t**2 * var_imu_w
    Q1=np.diag([a,a,a,b,b,b])

    # p_cov_predict= f_jac@p_cov_last@f_jac.T + l_jac@Q1@l_jac.T
    p_cov[k]= f_jac@p_cov[k-1]@f_jac.T + l_jac@Q1@l_jac.T
